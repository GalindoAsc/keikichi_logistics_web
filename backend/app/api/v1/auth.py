...
from app.schemas.auth import LoginRequest, RegisterRequest, Token, TokenRefresh, UserOut
...
async def login(payload: LoginRequest, db: AsyncSession = Depends(get_db_session)):
    ...
    refresh_token = create_access_token(
        str(user.id), expires_minutes=settings.refresh_token_expire_days * 24 * 60
    )
    return Token(
