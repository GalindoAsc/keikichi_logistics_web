...
/usr/bin/expect <<EOF
set timeout -1
spawn ssh -p $NAS_PORT -t $NAS_USER@$NAS_HOST "cd $NAS_DIR && echo -e '${GREEN}>>> Pulling latest changes...${NC}' && git pull && echo -e '${GREEN}>>> Executing deploy script...${NC}' && chmod +x deploy.sh && ./deploy.sh deploy"
expect {
